...
const app = express();
const JWT_SECRET = process.env.JWT_SECRET || 'super_secret_gestor_gastos_key_2026';
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error interno del servidor: ' + (error.message || 'Desconocido') });
  }
});
...
app.post('/api/expenses', authenticate, async (req, res) => {
  const { title, amount, category, payment_method, card_name, date, pay_next_month, is_recurring, installment_total } = req.body;
  ...
  try {
    const result = await pool.query(
      `UPDATE expenses
       ...
       RETURNING *`,
      [title, parseFloat(amount), category || 'General', payment_method, card_name || '', date, impactDateStr, isPayNextMonth, Boolean(is_recurring), id, req.user.id]
    );
